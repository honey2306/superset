#!/bin/bash
# Superset agent notification hook v3
...
if [ "$DEBUG_HOOKS_ENABLED" = "1" ]; then
  STATUS_CODE=$(curl -sG "http://127.0.0.1:${SUPERSET_PORT:-3026}/hook/complete" \
  ...
  debug_log "v1 status=$STATUS_CODE port=${SUPERSET_PORT:-3026}"
else
  debug_log "v1 dispatch port=${SUPERSET_PORT:-3026}"
  curl -sG "http://127.0.0.1:${SUPERSET_PORT:-3026}/hook/complete" \
    --connect-timeout 1 --max-time 2 \
    --data-urlencode "paneId=$SUPERSET_PANE_ID" \
    --data-urlencode "tabId=$SUPERSET_TAB_ID" \
    --data-urlencode "workspaceId=$SUPERSET_WORKSPACE_ID" \
