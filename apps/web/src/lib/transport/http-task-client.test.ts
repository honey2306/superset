...
describe("AutoMateHttpTaskClient", () => {
	...
	test("posts task input with the task token in a header and unwraps data", async () => {
		...
		const client = new AutoMateHttpTaskClient(
		...
		expect(await client.run({ op: "pull" })).toEqual({ ok: true });
	...
	test("redacts the task token from relay business errors", async () => {
		...
		const failure = client.run({ op: "health" });
	...
	test("default factory uses the HTTP task client", () => {
		...
		).toBeInstanceOf(AutoMateHttpTaskClient);
	});
});
