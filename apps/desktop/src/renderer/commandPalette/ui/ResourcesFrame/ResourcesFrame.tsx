...
export function ResourcesFrame() {
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			setExpandedIds((previous) => {
				...
				if (event.key === "ArrowRight") next.add(workspaceId);
	...
	const rows = useMemo<ResourceRow[]>(() => {
		...
		return sorted.flatMap((workspace) => {
			...
			const matchingSessions = workspace.sessions.filter((entry) =>
				getPaneName(entry).toLowerCase().includes(query),
			);
								...
								<Fragment key={workspace.workspaceId}>
									...
									{isExpanded &&
										sessions.map((entry) => (
											<CommandItem
												key={entry.sessionId}
												value={`resources ${workspace.workspaceId} ${entry.sessionId}`}
												{...{ [WORKSPACE_ID_ATTR]: workspace.workspaceId }}
												onSelect={() =>
													navigateToPane(workspace.workspaceId, entry.paneId)
												}
