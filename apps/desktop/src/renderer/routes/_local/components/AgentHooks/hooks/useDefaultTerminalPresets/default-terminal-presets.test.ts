...
describe("createDefaultTerminalPresetRows", () => {
	it("creates linked default presets for an empty preset list", () => {
		let id = 0;
		const rows = createDefaultTerminalPresetRows({
			agents: [
				...
				createAgent({ presetId: "amp", order: 8 }),
			],
			existingPresets: [],
			createId: () =>
		...
		expect(rows.map((row) => row.tabOrder)).toEqual([0, 1, 2, 3, 4, 5, 6, 7]);
		...
		expect(rows[7]?.commands).toEqual(["dsh-acp-demo"]);
	});
	...
	it("does not seed when presets already exist", () => {
		const existingPreset = {
			...
			tabOrder: 0,
			createdAt,
		} satisfies TerminalPresetRow;

		expect(
			createDefaultTerminalPresetRows({
