...
export const workspaceLocalStateSchema = z.object({
	...
	// It stays optional so older rows can use the display-name fallback without
	// being rewritten as an authoritative identity field.
...
// projectIds uses plain z.string() (not uuid) because persisted project IDs
...
export const USER_PREFERENCES_ID = "preferences" as const;

export const DEFAULT_USER_PREFERENCES: UserPreferencesRow = {
...
export function healWorkspaceLocalState(raw: unknown): WorkspaceLocalStateRow {
	...
	const current = { ...r } as Partial<WorkspaceLocalStateRow> &
		Record<string, unknown>;
	delete current.pendingMigratedTerminals;
	return {
		...current,
...
export function healUserPreferences(raw: unknown): UserPreferencesRow {
	...
	return {
		...
		...r,
		fileLinks: { ...DEFAULT_USER_PREFERENCES.fileLinks, ...r.fileLinks },
		urlLinks: { ...DEFAULT_USER_PREFERENCES.urlLinks, ...r.urlLinks },
