...
export function CommandsEditor({
	...
	const resolvedPlaceholder = placeholder ?? t("terminal.command");
	...
	const inputClassName =
		"h-8 flex-1 min-w-0 border-line/70 bg-transparent px-2 text-sm shadow-none dark:bg-transparent focus-visible:border-foreground/30 focus-visible:ring-2 focus-visible:ring-foreground/10";
	...
	return (
		<div className="flex flex-col gap-1.5 min-w-0">
				...
				>
					...
					{commands.length > 1 && (
						<Button
							...
							className="h-8 w-8 shrink-0 text-fg-mute opacity-0 transition-opacity hover:bg-destructive/10 hover:text-destructive focus-visible:opacity-100 group-hover/command-row:opacity-100 group-focus-within/command-row:opacity-100"
							aria-label={t("terminal.deleteCommand", { number: index + 1 })}
						>
			...
			<button
				...
				className="mt-1 inline-flex w-fit items-center gap-1.5 text-sm text-fg-mute transition-colors hover:text-fg focus-visible:text-fg focus-visible:outline-none"
			>
