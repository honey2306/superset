...
export function SetupProjectModal({
	...
	const runClone = async () => {
		...
		if (!trimmed) {
			toast.error(t("project.pickParentDirectory"));
			return;
		}
		setWorking(true);
		try {
	...
	const runImport = async () => {
		...
		if (!trimmed) {
			toast.error(t("project.pickProjectLocation"));
			return;
		}
		setWorking(true);
		try {
		...
		<Dialog open={open} onOpenChange={handleOpenChange} modal>
			<DialogContent className="max-w-[480px]">
				<DialogHeader>
					...
					<DialogDescription>{t("project.setupDescription")}</DialogDescription>
				</DialogHeader>
							...
							<>
								{repoCloneUrl && (
									<div className="flex flex-col gap-1">
										<Label className="text-xs">{t("project.repository")}</Label>
								...
								<div className="flex flex-col gap-1.5">
									...
									<div className="flex gap-1.5">
										<Input
											...
											placeholder={t("project.pickFolder")}
											disabled={working}
