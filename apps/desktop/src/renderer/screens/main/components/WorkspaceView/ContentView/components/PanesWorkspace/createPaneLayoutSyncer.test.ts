...
/**
 * The syncer is the pure, testable core of the Host-backed panes persistence
 ...
 * echo loop. The hook (`usePanesWorkspacePaneLayout`) wires it to the
 * `workspaceLocalState` TanStack DB collection; these tests inject an
...
describe("createPaneLayoutSyncer", () => {
	test("hydrates the store from persisted layout on hydrate()", () => {
		...
		const persisted: WorkspaceState<PanesPaneData> = {
			version: 1,
			tabs: [],
			activeTabId: null,
		};
		...
		const persistedLayout: WorkspaceState<PanesPaneData> = {
		...
		const readValue: WorkspaceState<PanesPaneData> | null = persistedLayout;
		const writeCalls: WorkspaceState<PanesPaneData>[] = [];
		const syncer = createPaneLayoutSyncer<PanesPaneData>({
			store,
	...
	test("does not replace a local tab with a stale persisted snapshot", () => {
		const store = createWorkspaceStore<PanesPaneData>();
		let persisted: WorkspaceState<PanesPaneData> = EMPTY_STATE;
		const syncer = createPaneLayoutSyncer<PanesPaneData>({
			store,
	...
	test("resetSyncMarker prevents the next hydrate from being suppressed as an echo", () => {
		const store = createWorkspaceStore<PanesPaneData>();
		let readValue: WorkspaceState<PanesPaneData> | null = null;
		const syncer = createPaneLayoutSyncer<PanesPaneData>({
			store,
			readPersisted: () => readValue,
			writePersisted: () => {},
