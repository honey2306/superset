...
export interface V1PanesPresetAgentLaunch {
	terminalId: string;
...
export interface V1PanesResolvedPresetLaunch {
	initialCommand: string | undefined;
 ...
 */
export async function resolveV1PanesPresetLaunch(
	plan: V1PanesPresetAgentLaunch,
	launchAgent: (input: {
		terminalId: string;
		agent: AgentType;
	}) => Promise<void>,
): Promise<V1PanesResolvedPresetLaunch> {
	if (!plan.agentName) {
		return {
