...
/**
 * `buildTerminalContextMenu` is the pure, testable core of the panes
...
const DEFAULTS: ContextMenuActionConfig<PanesPaneData>[] = [
...
function build(defaults: ContextMenuActionConfig<PanesPaneData>[] = DEFAULTS) {
	return buildTerminalContextMenu(
		{
...
describe("buildTerminalContextMenu", () => {
	...
	test("kill-session action calls the injected killSession keyed by terminalId (backend identity)", () => {
		...
		const actions = buildTerminalContextMenu(
			{
				terminalRuntime: terminalRuntimeStub,
				killSession,
				labels: LABELS,
