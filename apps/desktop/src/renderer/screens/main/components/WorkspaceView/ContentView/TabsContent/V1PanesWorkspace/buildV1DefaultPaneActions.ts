...
export interface V1DefaultPaneActionsDeps {
	labels: {
		split: string;
		close: string;
	};
...
/**
 * Header pane actions for the v1-panes mount (split + close).
 *
 * Mirrors v2's `useDefaultPaneActions` (split-along-longer-side + close),
 * typed for `V1PanesPaneData` and terminal-only. `split` mints a fresh
 ...
 */
export function buildV1DefaultPaneActions(
	deps: V1DefaultPaneActionsDeps,
