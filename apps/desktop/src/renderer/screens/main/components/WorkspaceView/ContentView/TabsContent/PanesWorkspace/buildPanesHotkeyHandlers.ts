...
export interface PanesHotkeyHandlersDeps {
...
export interface PanesHotkeyHandlers {
...
/**
 * Hotkey handler implementations for the panes mount.
 *
 ...
 * `usePanesHotkeys` hook registers them via `useHotkey`. Held as a
 * builder so the handler logic is testable without the React effect
 * environment `useHotkey` needs.
 */
export function buildPanesHotkeyHandlers(
	deps: PanesHotkeyHandlersDeps,
): PanesHotkeyHandlers {
