...
const V1_PANES_PRESET_BAR_ITEM_TYPE = "V1_PANES_PRESET_BAR_ITEM";

interface V1PanesPresetBarItemProps {
...
interface V1PanesPresetDragItem {
	id: string;
	index: number;
...
export function V1PanesPresetBarItem({
	preset,
	pinnedIndex,
	isDark,
		...
		() => ({
			...
			item: () => {
				...
				return {
					...
					persisted: false,
				} satisfies V1PanesPresetDragItem;
			},
	...
	const [{ targetHandlerId }, drop] = useDrop(
		() => ({
			accept: V1_PANES_PRESET_BAR_ITEM_TYPE,
			hover: (item: V1PanesPresetDragItem) => {
			...
			drop: (item: V1PanesPresetDragItem) => {
