...
const DEPS = {
	...
	} as unknown as PaneRegistry<V1PanesPaneData>,
	labels: {
...
interface FakeCtx {
	pane: { id: string; kind: "terminal"; data: V1PanesPaneData };
	tab: { id: string };
	store: {
		getState: () => {
			tabs: never[];
...
describe("buildV1DefaultContextMenuActions", () => {
	test("split-down calls ctx.actions.split('down', terminal pane)", async () => {
		const actions = buildV1DefaultContextMenuActions(
			DEPS,
		) as ContextMenuActionConfig<V1PanesPaneData>[];
