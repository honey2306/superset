...
describe("openCommentInPanesStore", () => {
	test("adds a new tab with a comment pane seeded from the payload when none exists", () => {
		...
		const opened = openCommentInPanesStore(store, commentA);
		...
		expect(opened).toEqual({ tabId: state.tabs[0].id, paneId: pane?.id });
	});
	...
	test("reuses the existing comment pane: updates its data and activates it", () => {
		...
		// add a third tab.
		const opened = openCommentInPanesStore(store, commentB);
		...
		expect(opened).toEqual({ tabId: commentTab.id, paneId: commentPane?.id });
	});
