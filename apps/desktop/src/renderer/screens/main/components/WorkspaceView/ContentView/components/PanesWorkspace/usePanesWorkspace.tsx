...
import { shouldBlockFileClose } from "renderer/lib/panes";
import { killTerminalForPaneOrSession } from "renderer/lib/terminal/terminal-cleanup";
...
import { HostServiceTerminalPane } from "renderer/screens/main/components/WorkspaceView/ContentView/components/Terminal/HostServiceTerminalPane";
...
function usePanesRegistry(workspaceId: string): PaneRegistry<PanesPaneData> {
	...
	return useMemo<PaneRegistry<PanesPaneData>>(() => {
		...
		const fileViewer: PaneDefinition<PanesPaneData> = {
			onBeforeClose: (pane) => {
				const session = useEditorSessionsStore.getState().sessions[pane.id];
				const document = session
					? useEditorDocumentsStore.getState().documents[session.documentKey]
					: null;
				if (!shouldBlockFileClose(document?.dirty ?? false)) return true;
				useEditorSessionsStore
					.getState()
					.setPendingIntent(pane.id, { type: "close-pane" }, "unsaved");
				return false;
			},
