...
import {
	type PanesPresetTarget,
	planPanesPresetOpen,
} from "./planPanesPresetOpen";
import type { PanesPaneData } from "./types";

/**
 * When the "use ACP for agent presets" setting is on and the host supports ACP,
 * `PanesWorkspace` builds one of these and passes it through the opener chain.
...
/**
 * Unified pane-open entry for the panes mount.
 *
 ...
 * `useTabsWithPresets.openPreset` writes to the global tabs store; the panes
 * store now owns the view, so preset launch must route here instead.
 *
 ...
 * fidelity follow-up.
 */
export async function openPanesPreset(
	store: StoreApi<WorkspaceStore<PanesPaneData>>,
	preset: Pick<TerminalPreset, "commands" | "cwd" | "name">,
	options: { target: PanesPresetTarget; acpLauncher?: AcpPresetLauncher },
) {
	...
	const plan = planPanesPresetOpen(preset, {
...
export function usePanesPresetOpeners(
	_workspaceId: string,
	store: StoreApi<WorkspaceStore<PanesPaneData>>,
	...
	const openPreset = useCallback(
		(
			preset: Pick<TerminalPreset, "commands" | "cwd" | "name">,
			options: { target: PanesPresetTarget },
		) => openPanesPreset(store, preset, { ...options, acpLauncher }),
		[store, acpLauncher],
	);
...
export type PanesPresetOpeners = ReturnType<typeof usePanesPresetOpeners>;
