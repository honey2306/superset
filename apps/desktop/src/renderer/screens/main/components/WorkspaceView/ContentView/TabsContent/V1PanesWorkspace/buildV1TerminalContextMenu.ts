 ...
 */
export interface V1PanesTerminalContextMenuRuntime {
...
export interface V1PanesTerminalContextMenuLabels {
	copy: string;
	paste: string;
...
export interface V1PanesTerminalContextMenuHotkeys {
	clear: string | undefined;
	scrollToBottom: string | undefined;
}

export interface V1PanesTerminalContextMenuIcons {
	copy: ReactNode;
	paste: ReactNode;
	clear: ReactNode;
...
export interface V1PanesTerminalContextMenuDeps {
	terminalRuntime: V1PanesTerminalContextMenuRuntime;
	/** Kills a still-running terminal session without closing the pane. */
	killSession: (terminalId: string) => void;
	labels: V1PanesTerminalContextMenuLabels;
	hotkeys: V1PanesTerminalContextMenuHotkeys;
	icons: V1PanesTerminalContextMenuIcons;
	/** Optional platform prefix for the copy/paste shortcuts (e.g. "⌘" / "Ctrl+"). */
	copyPasteShortcutPrefix?: string;
}

/**
 * Build the terminal pane's context menu actions for the v1-panes mount.
 *
 ...
 */
export function buildV1TerminalContextMenu(
	deps: V1PanesTerminalContextMenuDeps,
	...
	const terminalActions: ContextMenuActionConfig<V1PanesPaneData>[] = [
		{
			key: "copy",
			label: labels.copy,
			icon: icons.copy,
	...
	const killAction: ContextMenuActionConfig<V1PanesPaneData> = {
