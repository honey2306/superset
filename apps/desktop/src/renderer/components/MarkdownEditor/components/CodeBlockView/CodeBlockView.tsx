			...
			>
				<DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
					<DropdownMenuTrigger asChild>
						<button
							type="button"
							className="flex items-center gap-1 h-6 px-2 text-xs bg-background/80 backdrop-blur border border-line rounded hover:bg-accent-tint transition-colors"
						>
							{currentLabel}
				...
				<button
					type="button"
					onClick={handleCopy}
					className="flex items-center justify-center h-6 w-6 bg-background/80 backdrop-blur border border-line rounded hover:bg-accent-tint transition-colors"
				>
					{copied ? (
