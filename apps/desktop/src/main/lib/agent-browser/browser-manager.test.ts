...
describe("AgentBrowserManager page ownership", () => {
	test("closes agent pages and their popups while preserving user pages", async () => {
		views.splice(0);
		...
		await browser.createPage("session", "https://next.example", "agent");
		await browser.closeAgentPages("session");
		expect(browser.getState("session").pages).toHaveLength(1);
	});
});
