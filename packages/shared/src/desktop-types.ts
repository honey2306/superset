...
/** Terminal preset persisted in desktop settings. */
export const terminalPresetSchema = z.object({
	...
	/** User-uploaded custom icon stored as a `data:image/*` URI. */
	icon: z.string().optional(),
});

export type TerminalPreset = z.infer<typeof terminalPresetSchema>;

/** External apps that can open a path. */
...
/** Absolute path launch request sent from the renderer to Electron. */
export const openInAppInputSchema = z.object({
	path: z.string(),
	app: z.enum(EXTERNAL_APPS),
	line: z.number().int().positive().optional(),
	column: z.number().int().positive().optional(),
});

export type OpenInAppInput = z.infer<typeof openInAppInputSchema>;
export type OpenInAppLocation = Pick<OpenInAppInput, "line" | "column">;

/** Apps that are not editors/IDEs and cannot be the global default editor. */
