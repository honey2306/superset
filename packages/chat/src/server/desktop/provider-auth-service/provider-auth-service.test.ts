...
const { ProviderAuthService } = await import("./provider-auth-service");

describe("ProviderAuthService OpenAI auth storage", () => {
	beforeEach(() => {
	...
	it("uses standalone createAuthStorage and reuses it across calls", async () => {
		...
		await providerAuthService.setOpenAIApiKey({ apiKey: " test-key " });
		await providerAuthService.getOpenAIAuthStatus();
		await providerAuthService.clearOpenAIApiKey();
	...
	it("stores and clears Anthropic API key in standalone auth storage", async () => {
		...
		await providerAuthService.setAnthropicApiKey({
		...
		await providerAuthService.clearAnthropicApiKey();
	...
	it("persists Anthropic OAuth credentials to auth storage on completion", async () => {
		...
		await providerAuthService.setAnthropicApiKey({ apiKey: " old-key " });

		const start = await providerAuthService.startAnthropicOAuth();
		expect(start.url).toContain("claude.ai/oauth/authorize");

		const result = await providerAuthService.completeAnthropicOAuth({
		...
		expect((await providerAuthService.getAnthropicAuthStatus()).method).toBe(
			"oauth",
		);
	});
	...
	it("prefers a managed Anthropic API key over env-config credentials", async () => {
		...
		await providerAuthService.setAnthropicEnvConfig({
			envText:
		...
		expect(process.env.ANTHROPIC_API_KEY).toBeUndefined();
		expect(process.env.ANTHROPIC_AUTH_TOKEN).toBeUndefined();
		expect(await providerAuthService.getAnthropicAuthStatus()).toEqual({
			authenticated: true,
			method: "api_key",
			source: "managed",
			issue: null,
	...
	it("disconnects managed Anthropic OAuth", async () => {
		...
		await providerAuthService.disconnectAnthropicOAuth();
	...
	it("saves Anthropic gateway env config and resolves it through managed auth storage", async () => {
		...
		expect(providerAuthService.getAnthropicEnvConfig()).toEqual({
			envText:
				"ANTHROPIC_BASE_URL=https://ai-gateway.vercel.sh\nANTHROPIC_AUTH_TOKEN=gateway-token",
			variables: {
				ANTHROPIC_BASE_URL: "https://ai-gateway.vercel.sh",
	...
	it("clears Anthropic gateway env vars", async () => {
		...
		await providerAuthService.clearAnthropicEnvConfig();
		...
		expect(
			(await providerAuthService.getAnthropicAuthStatus()).method,
		).toBeNull();
	});
	...
	it("starts and completes OpenAI OAuth via auth storage login", async () => {
		...
		const start = await providerAuthService.startOpenAIOAuth();
		expect(start.url).toContain("auth.openai.com");
		expect(start.instructions).toContain("Open browser");

		await providerAuthService.completeOpenAIOAuth({ code: "code#state" });
		expect(fakeAuthStorage.login).toHaveBeenCalledWith(
			"openai-codex",
			expect.any(Object),
		);
	...
	it("replaces OpenAI API key auth with OAuth when OAuth completes", async () => {
		...
		await providerAuthService.setOpenAIApiKey({ apiKey: " managed-key " });

		await providerAuthService.startOpenAIOAuth();
		await providerAuthService.completeOpenAIOAuth({ code: "code#state" });
		const status = await providerAuthService.getOpenAIAuthStatus();
	...
	it("recognizes legacy OpenAI auth storage entries", async () => {
		...
		expect(await providerAuthService.getOpenAIAuthStatus()).toEqual({
			authenticated: true,
			method: "oauth",
			source: "managed",
			issue: null,
	...
	it("disconnects managed OpenAI OAuth", async () => {
		...
		await providerAuthService.disconnectOpenAIOAuth();
	...
	it("completes OpenAI OAuth when provider flow does not require manual code", async () => {
		...
		try {
			const start = await providerAuthService.startOpenAIOAuth();
			expect(start.url).toContain("auth.openai.com");
			await providerAuthService.completeOpenAIOAuth({});
	...
	it("clears OpenAI OAuth session when auth-url wait times out", async () => {
		...
		const timeoutSlot = ProviderAuthService as unknown as {
		...
		try {
			await expect(providerAuthService.startOpenAIOAuth()).rejects.toThrow(
				"Timed out while waiting for OpenAI OAuth URL",
			);
			expect(loginSignal?.aborted).toBe(true);
			await expect(
				providerAuthService.completeOpenAIOAuth({ code: "code#state" }),
