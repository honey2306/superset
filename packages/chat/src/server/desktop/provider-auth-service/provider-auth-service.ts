...
export class ProviderAuthService {
	...
	async getAnthropicAuthStatus(): Promise<AuthStatus> {
		...
		if (
			...
			try {
				...
				console.warn(
					"[provider-auth-service] Anthropic OAuth refresh failed, falling back to expired state:",
					error,
				);
			}
		}
	...
	private getOpenAIOAuthFlowOptions(): OAuthFlowOptions {
		return {
			...
			providerName: "OpenAI",
			sessionSlot: "openai",
			ttlMs: ProviderAuthService.OPENAI_AUTH_SESSION_TTL_MS,
			urlTimeoutMs: ProviderAuthService.OAUTH_URL_TIMEOUT_MS,
			expiredMessage:
	...
	private getAnthropicOAuthFlowOptions(): OAuthFlowOptions {
		return {
			...
			ttlMs: ProviderAuthService.ANTHROPIC_AUTH_SESSION_TTL_MS,
			urlTimeoutMs: ProviderAuthService.OAUTH_URL_TIMEOUT_MS,
			expiredMessage:
	...
	private logAuthResolution(
		...
		console.info("[provider-auth-service][auth-resolution]", {
			provider,
			...details,
		});
	}
