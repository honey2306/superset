...
function normalizeAnthropicBaseUrl(
	...
	try {
		...
		console.warn(
			"[provider-auth-service][anthropic-env] Invalid ANTHROPIC_BASE_URL; using raw value.",
...
function readPersistedAnthropicEnvConfig(
	...
	try {
		...
		console.warn(
			"[provider-auth-service][anthropic-env] Failed to read persisted env config.",
			{
				configPath,
				error: error instanceof Error ? error.message : String(error),
			},
...
export function getAnthropicEnvConfig(
	...
	try {
		...
		console.warn(
			"[provider-auth-service][anthropic-env] Persisted env config is invalid and will be ignored.",
			{
