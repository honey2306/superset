...
const COMPUTER_USE_INSTRUCTIONS = [
	...
	"For semantic commands such as Save, inspect computer_menu list and prefer the observed exact menu path over raw keyboard chords. For commands that require the app to be active, focus the pinned process and use foreground menu access; background delivery alone may not execute the command. Confirm the intended document first, and verify completion (for Save, persisted content), not only AX text or key delivery. After an input invalidates a snapshot, obtain a fresh observation before another element action.",
