...
describe("ACP agent dispatch selection", () => {
	it("uses ACP only when enabled and the resolved preset has an ACP harness", () => {
		expect(shouldRunAgentWithAcp(false, "myflicker")).toBe(false);
		expect(shouldRunAgentWithAcp(true, "myflicker")).toBe(true);
		expect(shouldRunAgentWithAcp(true, "amp")).toBe(false);
		expect(getAcpHarnessForPreset("myflicker")).toBe("myflicker-acp");
	});
});
