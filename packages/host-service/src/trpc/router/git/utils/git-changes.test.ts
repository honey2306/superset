...
describe("parseGitLog", () => {
	...
	test("preserves commas in decorations from the worker separator", () => {
		const [entry] = parseGitLog(
			"abc123\x1fabc123\x1fparent\x1fHEAD -> feature,comma\x1dtag: release,1\x1frefs/heads/feature,comma\x1fcomma ref\x1fAuthor\x1f1700000000\x1e",
		);

		expect(entry).toMatchObject({
			refs: ["HEAD -> feature,comma", "tag: release,1"],
			branch: "feature,comma",
		});
	});
