	...
	return (
		<div className="tool-block">
			<div className="head">
				...
				<span className="status">+{addLines.length} −{delLines.length}</span>
			</div>
			<div className="body" style={{ fontFamily: "var(--font-mono)" }}>
				{ctxLines.map((l, i) => (
					<div key={`c${i}`} style={{ paddingLeft: 12 }}>{l}</div>
				))}
				{delLines.map((l, i) => (
					<div key={`d${i}`} className="diff-line del">{l}</div>
				))}
				{addLines.map((l, i) => (
					<div key={`a${i}`} className="diff-line add">{l}</div>
				))}
			</div>
		</div>
	);
	...
	return (
		<div className="chat">
			<div className="chat-stream">
				...
				<AsstMsg>
					<div className="prose">
						我先读一下 coordinator 的关停逻辑,再对比 pty session 的引用计数——如果两边都在等对方 flush,就是典型循环等待。
					</div>
					<ExploringGroup
						rows={[
							{ icon: "file", verb: "Reading", path: "host-service-coordinator.ts" },
							{ icon: "file", verb: "Reading", path: "session.ts" },
							{ icon: "search", verb: "Grep", path: "coordinator\\.shutdown" },
							{ icon: "changes", verb: "Listing", path: "apps/desktop/src/main/terminal-host/" },
						]}
					/>
					<div className="prose" style={{ paddingTop: 4 }}>
						找到了:<code style={{ fontFamily: "var(--font-mono)", background: "var(--hover)", padding: "1px 4px", borderRadius: 4 }}>coordinator.shutdown()</code> 里 <code style={{ fontFamily: "var(--font-mono)", background: "var(--hover)", padding: "1px 4px", borderRadius: 4 }}>await session.dispose()</code> 会在 pty 已经在等 coordinator 释放锁时死锁。把 dispose 改成 fire-and-forget + 5s 兜底 timeout 即可。
					</div>
					<BashTool
					...
					/>
					<div className="prose">测试跑通了。要一起把 timeout 值放到 <code style={{ fontFamily: "var(--font-mono)", background: "var(--hover)", padding: "1px 4px", borderRadius: 4 }}>settings.ptyShutdownGraceMs</code> 让用户可调吗?</div>
				</AsstMsg>
				<UserMsg text="先不用,合进去看看再说。@config-owner 帮我 review 一下 timeout 值。" />
				<AsstMsg streaming>
					<div className="prose">已经开好 PR,正在等 lint pass。5s 是我看 pty-subprocess-ipc 里默认的 kill grace…</div>
